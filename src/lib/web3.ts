...
export async function createTokenCreationTransaction(
  ...
  try {
    ...
    console.log('ATA', newATA.toString(), 'total supply', BigInt(tokenMetaData.supply) * BigInt(10 ** tokenMetaData.decimals));
    ...
    if (tokenMetaData.mintable) {
      transaction.add(
        createSetAuthorityInstruction(tokenMint, publicKey, AuthorityType.MintTokens, null, undefined, TOKEN_2022_PROGRAM_ID),
      );
    }
